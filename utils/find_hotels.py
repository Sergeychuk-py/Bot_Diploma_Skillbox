...
def find_and_show_hotels(message: Message, data: Dict) -> None:
    ...
    response_hotels = api.main_request.request('POST', url, payload)
    ...
    if response_hotels.status_code == 200:
        ...
        for hotel in hotels.values():
            ...
            if count < int(data['quantity_hotels']):
                ...
                get_summary = api.main_request.request('POST', summary_url, summary_payload)
    ...
    bot.send_message(message.chat.id, "Поиск окончен!Для нового запроса введите команду '/help'")
